...
function buildWelcomeText() {
  return [
    "Привет! Я AI-ассистент ainur.",
    "",
    "Мы собираем любые цифровые решения: сайты, боты, автоматизации, интеграции, платформы, AI-системы. Рабочее решение — за 3–7 дней.",
    "",
    "Могу помочь:",
    "1. Подсказать, какое решение подойдёт под вашу задачу.",
    "2. Собрать информацию для заявки.",
    "",
    "Выбирайте 👇",
  ].join("\n");
}
...
async function completeLead(session: TelegramAssistantSession, message: TelegramMessage) {
  ...
  const reply = "Спасибо! Заявка передана. Свяжусь в ближайшее время. Если хотите — можете ещё задать вопрос.";
  await addChatMessage(String(message.chat.id), "assistant", reply);
  await sendTelegramMessage(message.chat.id, reply, buildKeyboard(startKeyboard));
}
...
async function handleLeadMode(session: TelegramAssistantSession, message: TelegramMessage, text: string) {
  ...
  if (isQuestion(text)) {
    ...
    const kbAnswer = await askLLM(text, history);
...
export async function handleTelegramUpdate(update: TelegramUpdate) {
  ...
  if (text === "Задать вопрос") {
    ...
    await sendTelegramMessage(
      message.chat.id,
      "Спрашивай! Расскажу про услуги, сроки, цены — или подскажу, какое решение подойдёт под задачу.",
      buildKeyboard(startKeyboard),
    );
    return;
  }
